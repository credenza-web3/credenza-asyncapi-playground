...
<section>
        ...
        <tbody>
          {#each subscriptions as subscription}
            <tr>
              ...
              <td>
                <input
                  bind:checked={subscription.once}
                  onchange={() => reSubscribe(subscription.id)}
                  type="checkbox"
                />
              </td>
              <td>
                <input
                  bind:checked={subscription.active}
                  onchange={() => reSubscribe(subscription.id)}
                  type="checkbox"
                />
              </td>
  ...
  <section class="flex w-full justify-end pt-1">
    <button class="btn w-full" class:btn-sm={subscriptions.length} onclick={addSubscription}
